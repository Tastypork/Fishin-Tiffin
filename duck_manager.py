...
DATA_DIR = Path(__file__).resolve().parent / "duck_data"
DATA_DIR.mkdir(parents=True, exist_ok=True)
HTML_DIR = DATA_DIR / "html"
HTML_DIR.mkdir(parents=True, exist_ok=True)


NAMES_COMMON_PATH = DATA_DIR / "names_common.json"      # used by Common/Uncommon/Rare
NAMES_LEGENDARY_PATH = DATA_DIR / "names_legendary.json"  # used by Legendary/Mythic
...
def build_duck_onboarding_embed(role_mention: str) -> discord.Embed:
    rarity_lines = [f"- **{label}**: {weight}%" for label, weight in RARITY_WEIGHTS]
    embed = discord.Embed(
        ...
        description=(
            ...
            "- Catches roll rarity, stats, and a tiny shiny chance.\n"
            ...
            "Use `!help` anytime for full commands and mechanics."
        ),
    ...
    async def duck_help(self, ctx: commands.Context):
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                "**How to Play**\n"
                "Use `!duck` to catch one duck per cooldown. Every catch rolls rarity, stats, and shiny chance.\n\n"
                "**Rarity Odds**\n"
                f"{chr(10).join(rarity_lines)}\n\n"
                "**Core Mechanics**\n"
                "- **Cooldown** is randomized after every successful catch.\n"
                "- **Stats** (attack/defense/speed) increases by rarity.\n"
                "- **Shiny** ducks are cosmetic and very rare.\n"
                "- **Theft**: Small chance on `!duck` to steal a random duck from another player.\n"
